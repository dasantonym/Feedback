...
void Caustics::setup(int width, int height, int maxNumPoints, float scaleFactor) {
	cleanUp();
    intensity = 0.0;
    mainWidth = width;
    mainHeight = height;
	...
	ping = new ofFbo();
	pong = new ofFbo();
	ping->allocate(mainWidth, mainHeight, GL_RGB16F);
	pong->allocate(mainWidth, mainHeight, GL_RGB16F);
...
void Caustics::addPoint(const ofVec2f &p) {
	triangulator.addPoint(p.x*scale, p.y*scale);
}

void Caustics::update(float intense) {
    intensity = intense;
...
void Caustics::renderToFbo() {
	...
	ping->begin();
	{
		...
		glPushMatrix();
		glTranslatef(mainWidth/2, mainHeight/2, 0);
		float s = 1 + 0.002;
